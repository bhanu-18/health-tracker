...
/**
 * Densities in grams per millilitre, for ingredients measured by volume.
 *
 * Supplied only where the value is genuinely standard. Where omitted, the food
 * does not offer volume units at all -- which is the honest outcome. A "cup of
 * chopped carrot" varies far too much with chop size to convert, and inventing
 * a figure is precisely how a recipe total becomes confidently wrong.
 */
const DENSITY = {
  water: 1.0,
  milk: 1.03,
  ...
  honey: 1.42,
  soySauce: 1.15,
  /** Dry grains and pulses, uncooked. */
  grainDry: 0.85,
  /** Sifted flours -- notably light: a cup of atta is about 127 g, not 240. */
  flour: 0.53,
  sugar: 0.85,
  salt: 1.2,
  /** Ground spices. */
...
const per100g = (
  ...
  ing({
    ...
    cuisine,
    densityGPerMl,
    // Measured by weight or volume, never counted.
    isCountable: false,
  });
...
export const SEED_INGREDIENTS: NewFood[] = [
  ...
  per100g('ing-basmati-raw', 'Basmati rice, raw', 350, 7.5, 78, 0.9, 'Grain', DENSITY.grainDry),
  per100g('ing-rice-white-raw', 'White rice, raw', 345, 6.8, 78, 0.5, 'Grain', DENSITY.grainDry),
  per100g('ing-rice-brown-raw', 'Brown rice, raw', 362, 7.9, 76, 2.9, 'Grain', DENSITY.grainDry),
  per100g('ing-poha', 'Poha (flattened rice)', 350, 6.6, 77, 1.2, 'Grain', DENSITY.poha),
  per100g('ing-rava', 'Rava (semolina)', 360, 10.4, 74, 1.1, 'Grain', DENSITY.rava),
  per100g('ing-atta', 'Wheat flour (atta)', 340, 12, 71, 1.7, 'Grain', DENSITY.flour),
  per100g('ing-maida', 'Maida (refined flour)', 348, 10, 74, 1, 'Grain', DENSITY.flour),
  per100g('ing-besan', 'Besan (gram flour)', 387, 22, 58, 6.7, 'Grain', DENSITY.flour),
  per100g('ing-oats', 'Oats, rolled', 389, 16.9, 66, 6.9, 'Grain', DENSITY.oats),
  per100g('ing-quinoa', 'Quinoa, raw', 368, 14.1, 64, 6.1, 'Grain', DENSITY.grainDry),
  ...
  per100g('ing-toor-dal', 'Toor dal, dry', 343, 22, 62, 1.7, 'Legume', DENSITY.grainDry),
  per100g('ing-moong-dal', 'Moong dal, dry', 347, 24, 59, 1.2, 'Legume', DENSITY.grainDry),
  per100g('ing-chana-dal', 'Chana dal, dry', 360, 20, 60, 5.6, 'Legume', DENSITY.grainDry),
  per100g('ing-urad-dal', 'Urad dal, dry', 341, 25, 59, 1.6, 'Legume', DENSITY.grainDry),
  per100g('ing-masoor-dal', 'Masoor dal, dry', 352, 25, 60, 1.1, 'Legume', DENSITY.grainDry),
  per100g(
    'ing-rajma-dry',
    'Rajma (kidney beans), dry',
    333,
    24,
    60,
    0.8,
  ...
  per100g('ing-kala-chana', 'Kala chana, dry', 360, 20, 61, 5, 'Legume', DENSITY.grainDry),
  ...
  // --- Dairy and eggs ------------------------------------------------------
  per100g('ing-milk-whole', 'Milk, whole', 61, 3.2, 4.8, 3.3, 'Dairy', DENSITY.milk),
  per100g('ing-milk-toned', 'Milk, toned', 50, 3.1, 4.7, 2, 'Dairy', DENSITY.milk),
  per100g('ing-curd', 'Curd (dahi)', 60, 3.5, 4.7, 3.3, 'Dairy', DENSITY.yogurt),
  per100g('ing-greek-yogurt', 'Greek yogurt, plain', 59, 10, 3.6, 0.4, 'Dairy', DENSITY.yogurt),
  per100g('ing-yogurt-lowfat', 'Yogurt, low fat', 63, 5.3, 7, 1.6, 'Dairy', DENSITY.yogurt),
  per100g('ing-paneer-raw', 'Paneer', 321, 18, 3.6, 25, 'Dairy'),
  per100g('ing-cream', 'Fresh cream', 292, 2.1, 2.8, 30, 'Dairy', DENSITY.cream),
  perPiece('ing-cheese-slice', 'Cheese slice', '1 slice (20 g)', 20, 70, 4, 1, 5.5, 'Dairy'),
  perPiece('ing-butter', 'Butter', '1 tsp (5 g)', 5, 36, 0, 0, 4.1, 'Dairy', DENSITY.ghee),
  perPiece('ing-ghee-tsp', 'Ghee', '1 tsp (5 g)', 5, 45, 0, 0, 5, 'Dairy', DENSITY.ghee),
  ...
  per100g('ing-sesame-seeds', 'Sesame seeds', 573, 18, 23, 50, 'Nut', DENSITY.seeds),
  per100g('ing-flax-seeds', 'Flax seeds', 534, 18, 29, 42, 'Nut', DENSITY.seeds),
  per100g('ing-chia-seeds', 'Chia seeds', 486, 17, 42, 31, 'Nut', DENSITY.seeds),
  ...
  perPiece('ing-vinegar', 'Vinegar', '1 tbsp (15 g)', 15, 3, 0, 0.1, 0, 'Condiment', DENSITY.water),
  perPiece(
    'ing-ketchup',
    'Tomato ketchup',
    '1 tbsp (17 g)',
  ...
  per100g('ing-salt', 'Salt', 0, 0, 0, 0, 'Spice', DENSITY.salt),

  // --- Sweeteners ----------------------------------------------------------
  perPiece('ing-sugar', 'Sugar', '1 tsp (4 g)', 4, 16, 0, 4, 0, 'Sweetener', DENSITY.sugar),
  per100g('ing-jaggery', 'Jaggery', 383, 0.4, 98, 0.1, 'Sweetener', DENSITY.sugar),
  perPiece('ing-honey', 'Honey', '1 tbsp (21 g)', 21, 64, 0.1, 17, 0, 'Sweetener', DENSITY.honey),
